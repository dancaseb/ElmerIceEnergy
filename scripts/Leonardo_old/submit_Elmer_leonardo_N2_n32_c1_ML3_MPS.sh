...
#SBATCH --exclusive
#SBATCH --mem=0
#SBATCH --job-name=run_Elmer_leonardo_N2_n32_c1_ML3_MPS
...
start=$(date +%s)
srun -n ${SLURM_NTASKS} --cpu-bind=cores --cpus-per-task=${SLURM_CPUS_PER_TASK} ${SCRIPTSDIR}/Leonardo/wrapper-start.sh SSA_amgx_ML3.sif 
#srun -n ${SLURM_NTASKS} --cpu-bind=cores --cpus-per-task=${SLURM_CPUS_PER_TASK} bash -c "source ${SCRIPTSDIR}/Leonardo/wrapper-start.sh; echo 'done'"
srun -n $SLURM_NTASKS ${SCRIPTSDIR}/Leonardo/wrapper-stop.sh
#srun -n $SLURM_NTASKS --cpus-per-task=$SLURM_CPUS_PER_TASK --cpu-bind=cores bash -c "source ${SCRIPTSDIR}/Leonardo/wrapper-stop.sh; echo 'done'"
